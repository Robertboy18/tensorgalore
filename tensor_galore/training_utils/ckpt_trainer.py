...
class CheckpointTrainer:
    ...
    def resume_state_from_dir(self, save_dir):
        ...
        print(f"{self.optimizer.state_dict=}")
        # returns model, loads other modules in-place if provided
        self.model, self.optimizer, self.scheduler, self.regularizer, \
            self.start_epoch = load_training_state(save_dir=save_dir, save_name=save_name,
                                                model=self.model,
        ...
        if wandb_available:
            if wandb.run is not None:
                if wandb.run.step != self.start_epoch:
                    self.start_epoch = wandb.run.step
